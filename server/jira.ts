...
export class JiraService {
  ...
  public async searchIssuesByPhone(phone: string): Promise<any[]> {
    if (!phone) return [];
    ...
    try {
      // Build JQL to search phone across text fields
      const jql = `text ~ "${cleanPhone}" OR description ~ "${cleanPhone}" OR summary ~ "${cleanPhone}" order by updated DESC`;
      const results = await this.request(`/rest/api/3/search?jql=${encodeURIComponent(jql)}&maxResults=10&fields=summary,description,status,priority,assignee,created,updated`);
      const issues = (results.issues || []).map((data: any) => {
        const fields = data.fields || {};
        const statusName = fields.status?.name || "Unknown";
        const categoryName = fields.status?.statusCategory?.name || "To Do";
        const isToDo = /^(a\s*fazer|to\s*do|backlog|aberta|open|nova|new)$/i.test(statusName.trim());
        return {
          id: data.id,
          key: data.key,
          ...
          isNotToDo: !isToDo && !/^(done|conclu[ií]d[oa]|fechad[oa]|resolved|cancelad[oa])$/i.test(statusName.trim())
        };
      });
      return issues;
    } catch (err: any) {
      console.warn("[JiraService] JQL phone search failed:", err.message);
      return [];
    }
  }
