...
export async function agendaCompatHandler(req, res, subRoute) {
  ...
  if (subRoute === "vault" || subRoute === "agenda-vault") {
    // 32-byte deterministic fallback key in base64 (44 characters)
    const mockKey = process.env.AGENDA_VAULT_KEY || "U29sYXJBZ2VuZGFWYXVsdEtleTIwMjZTZWN1cmVBYmM=";
  ...
  if (subRoute === "tts" || subRoute === "agenda-tts") {
    ...
    const synthesizeDeepgramAura = async (textToSpeak, voiceModel, dKey) => {
      const v = voiceModel || "aura-orion-en";
  ...
  if (subRoute === "cases" || subRoute === "agenda-cases") {
    ...
    return sendResponse(res, 200, { id: body?.id || "case-" + Date.now(), created_at: new Date().toISOString(), ...(body || {}) });
  }
