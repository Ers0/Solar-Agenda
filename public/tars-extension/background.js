...
async function reportHyperflowConversationToObserver(snapshot, tabId = null) {
  ...
  const protocol = conversation.protocol || null;

  const events = timeline.map((m, index) => {
    const messageId = m?.id || m?.messageId || `sync-${index}-${Date.now()}`;
    return {
      eventId: `hf:${messageId}`,
      ...
      event: {
        ...
        page: conversation.conversationUrl || 'https://conversas.hyperflow.global/'
      }
    };
  });
  ...
  if (!events.length) {
    events.push({
      eventId: `hf-sync:${conversationId || 'unknown'}:${Date.now()}`,
  ...
  const requestId = `tars-observer-sync-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  console.info('[TARS Observer] Hyperflow FULL SYNC POST START', {
    requestId, url, tabId, protocol, conversationId, messageCount: events.length
  });
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'X-TARS-Event': 'observer',
        'X-TARS-Request-Id': requestId,
        'X-TARS-Sync': 'hyperflow-full'
      },
      credentials: 'omit',
      cache: 'no-store',
    ...
    console.error('[TARS Observer] Hyperflow FULL SYNC POST NETWORK ERROR', result);
    return result;
  }
}
...
const HYPERFLOW_SYNC_STATE_KEY = 'tarsHyperflowLastObserverSync';
...
async function syncHyperflowDirtyConversations() {
  ...
  console.info('[TARS Observer] 5-minute Hyperflow sync', { tabs: tabs.length, conversations: synced, failed });
...
async function injectLearningObserver(tabId) {
  ...
  try {
    ...
    console.info('[TARS Workflow Learning] observer injected', { tabId });
    notifyPopup('Workflow Learning', 'observer injected into tab', { tabId });
    return { ok:true };
  } catch (error) {
    console.warn('[TARS Workflow Learning] observer injection failed', { tabId, error:String(error?.message || error) });
    notifyPopup('Workflow Learning', 'observer injection failed', { tabId, error:String(error?.message || error) });
...
async function observeTab(tabId, reason='tab') {
  ...
  try {
    ...
    console.info('[TARS Workflow Learning] site observation recorded', { tabId: tab.id, reason, url: String(tab.url || '') });
    notifyPopup('Workflow Learning', 'site observation recorded', { tabId: tab.id, reason, url: String(tab.url || '') });
...
async function queueObserverEvent(event, senderTabId = null, conversation = null) {
  const eventType = String(event?.eventType || 'OBSERVER_EVENT');
  const modes = await chrome.storage.local.get([OBSERVER_MODE_KEY, LEARNING_MODE_KEY]);
  const observerOn = modes[OBSERVER_MODE_KEY] === true;
  const learningOn = modes[LEARNING_MODE_KEY] === true;
  const isLearningEvent = /^(SITE_ACCESSED|PAGE_NAVIGATION|DOM_STRUCTURE_SNAPSHOT|WORKFLOW_|LEARNING_)/i.test(eventType);
  if (!observerOn && !(learningOn && isLearningEvent)) {
    console.info('[TARS Observer] event ignored because Observer Mode and Workflow Learning are OFF', { eventType });
    return false;
  }
...
async function flushObserverQueue() {
  ...
  try {
    ...
    notifyPopup('Observer', 'observation batch delivered', { count: batch.length, status: response.status });
  } catch (error) {
    observerQueue.unshift(...batch);
    if (observerQueue.length > 250) observerQueue.splice(0, observerQueue.length - 250);
    console.warn('[TARS Observer] delivery failed; queued locally', String(error?.message || error));
    notifyPopup('Observer', 'delivery failed; events kept locally', { error:String(error?.message || error), queued:observerQueue.length });
  }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg && msg.type === 'TARS_OBSERVER_SET_MODE') {
    (async () => {
      const enabled = !!msg.enabled;
      await chrome.storage.local.set({ [OBSERVER_MODE_KEY]: enabled });
      if (!enabled && !(await learningEnabled())) observerQueue = [];
      console.info('[TARS Observer] mode changed', { enabled, queuedAfterChange: observerQueue.length });
      notifyPopup('Observer', enabled ? 'observer mode is now ON' : 'observer mode is now OFF', { enabled, queueLength: observerQueue.length });
      return { ok: true, enabled, queueLength: observerQueue.length };
  ...
  if (msg && msg.type === 'TARS_OBSERVER_SET_LEARNING') {
    (async () => {
      ...
      if (enabled) {
        ...
        if (!(await observerEnabled())) { observerQueue = []; }
        console.info('[TARS Workflow Learning] disabled; future learning events will be ignored');
      }
      console.info('[TARS Workflow Learning] mode changed', { enabled });
      notifyPopup('Workflow Learning', enabled ? 'learning mode is now ON' : 'learning mode is now OFF', { enabled });
  ...
  if (msg && msg.type === 'TARS_OBSERVER_SYNC_HYPERFLOW') {
    syncHyperflowToObserver(msg.tabId || sender?.tab?.id || null)
      .then(sendResponse)
...
function friendlyHoymilesError(result) {
  const raw = String(result?.error || result?.message || '');
  if (result?.friendlyError) return result.friendlyError;
  if (raw === 'automation_disabled') return 'Automation did not start because Automation Safety is turned off.';
  if (raw === 'emergency_stopped') return 'Automation did not start because the Emergency Stop is active.';
  if (/observer_mode/i.test(raw)) return 'Automation did not start because Observer Mode is active. Turn Observer Mode off to allow customer actions.';
  if (/missing_email/i.test(raw)) return 'Automation stopped because the customer email address is missing.';
  if (/incomplete/i.test(raw)) return 'Automation stopped because some required customer information is missing.';
  if (/parent_organization_missing/i.test(raw)) return 'Automation stopped because the parent organization could not be selected.';
  if (/organization_not_created/i.test(raw)) return 'Automation stopped because Hoymiles did not confirm that the organization was created.';
  if (/contact_not_filled/i.test(raw)) return 'Automation stopped because the organization contact information could not be entered or verified.';
  ...
  if (msg && msg.type === 'HOYMILES_CREATE_REQUEST') {
    ...
    (async () => {
      ...
      if (automationSetting.tarsEmergencyStop === true) {
        ...
        return await publishHoymilesAutomationStatus({ ok:false, error:'emergency_stopped' });
      }
      if (automationSetting.tarsAutomationEnabled === false) {
        console.warn('[TARS Hoymiles BG] automation blocked by safety switch');
        return await publishHoymilesAutomationStatus({ ok:false, error:'automation_disabled' });
      }
      const data = msg.data;
      if (!data?.email) return await publishHoymilesAutomationStatus({ ok:false, error:'missing_email' });
      ...
      if (memory.tarsHoymilesLastEmail === String(data.email).trim().toLowerCase()) {
        return await publishHoymilesAutomationStatus({ ok:true, skipped:true, reason:'email_already_created' });
      }
      ...
      const status = await publishHoymilesAutomationStatus(result || { ok:false, error:'empty_automation_result' });
      sendResponse(status);
    }).catch(async error => {
      const status = await publishHoymilesAutomationStatus({ ok:false, error:String(error?.message || error) });
      sendResponse(status);
    });
    return true;
  }
