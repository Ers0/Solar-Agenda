...
const PROTOCOL = 1;
let dynamicAppOrigin = 'https://solar-agenda.vercel.app';
const APP_ORIGIN = dynamicAppOrigin;
...
let appTabId = null;
const registeredAppTabs = new Set();
...
async function findAllAppTabs() {
  const allTabs = [];
  const checkedIds = new Set();

  // 1. Check registered tabs
  for (const tid of Array.from(registeredAppTabs)) {
    try {
      ...
      registeredAppTabs.delete(tid);
    }
  }

  // 2. Query URL patterns
...
async function findAppTab() {
  const tabs = await findAllAppTabs();
...
async function openAppTab() {
  ...
  const targetUrl = dynamicAppOrigin || APP_ORIGIN;
...
function queueObserverEvent(event, senderTabId = null, conversation = null) {
  ...
  // Live broadcast to Solar Agenda tabs for Copilot Live Monitor
  try {
    broadcastToApp({
      action: event.eventType || 'COPILOT_FEEDBACK',
      type: 'COPILOT_LIVE_FEEDBACK',
      event: {
        ...event,
        case: hasCase ? resolvedCase : (event?.case || null)
      }
    });
  } catch (_) {}
}
...
const jiraNotifiedTimestamps = new Map();
const jiraNotificationLinks = new Map();
const driveNotificationLinks = new Map();

async function resolveAppOrigin() {
  if (dynamicAppOrigin && dynamicAppOrigin !== 'https://solar-agenda.vercel.app') {
    return dynamicAppOrigin;
  }
  try {
    const tabs = await findAllAppTabs();
    if (tabs.length && tabs[0].url) {
      const u = new URL(tabs[0].url);
      dynamicAppOrigin = u.origin;
      return u.origin;
    }
  } catch (_) {}
  return dynamicAppOrigin || 'https://solar-agenda.vercel.app';
}

async function uploadMediaToDrive(payload, senderTabId) {
  const { sourceUrl, filename, mimeType, mediaType, clientName, clientPhone, manufacturer, isWarranty, conversationId } = payload || {};
  if (!sourceUrl) return { ok: false, error: 'sourceUrl missing' };

  const appOrigin = await resolveAppOrigin();
  const endpoint = `${appOrigin}/api/drive/upload-media`;

  const st = await chrome.storage.local.get(['googleDriveAccessToken', 'tarsDriveToken']);
  const accessToken = st.googleDriveAccessToken || st.tarsDriveToken || '';
  ...
  try {
    ...
    if (!res.ok || !data.ok) {
      console.warn('[TARS Drive Ingestion] Server returned', res.status, data);
      return { ok: false, error: data.error || `HTTP ${res.status}`, audit: data.audit };
    }

    const audit = data.audit || {};
    const notifId = `tars-drive-${Date.now()}`;
    if (audit.webViewLink) {
      driveNotificationLinks.set(notifId, { url: audit.webViewLink });
    }

    // 1. Chrome Push Notification
    if (chrome.notifications && chrome.notifications.create) {
      ...
      chrome.notifications.create(notifId, {
        ...
        message: `${audit.filename || 'Arquivo'} salvo em:\n~${audit.manufacturer || manufacturer}~/Mês/${clientName || 'Cliente'}/${folder}`,
        priority: 1,
        buttons: audit.webViewLink ? [{ title: 'Abrir no Google Drive ↗' }] : []
      }, () => {});
    }

    // 2. Broadcast to Solar Agenda tabs
    const appTabs = await findAllAppTabs();
    for (const tab of appTabs) {
      if (tab && tab.id) {
        try {
          chrome.tabs.sendMessage(tab.id, {
            type: 'TARS_DRIVE_MEDIA_UPLOADED',
            audit
          }, () => { if (chrome.runtime.lastError) {} });
        } catch (_) {}
      }
    ...
    console.warn('[TARS Drive Ingestion] Upload error:', err);
    return { ok: false, error: String(err?.message || err) };
  }
}

async function checkAndNotifyJiraKanban(payload, senderTabId) {
  const { phone, customerName, protocol, conversationId } = payload || {};
  if (!phone) return { ok: true, activeIssues: [] };

  const cleanDigits = String(phone).replace(/\D/g, "");
  if (!cleanDigits || cleanDigits.length < 6) return { ok: true, activeIssues: [] };

  const appOrigin = await resolveAppOrigin();
  const searchUrl = `${appOrigin}/api/jira/search-by-phone?phone=${encodeURIComponent(phone)}`;
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[TARS Jira Kanban] Search endpoint returned', res.status);
    ...
    if (activeIssues.length > 0) {
      for (const issue of activeIssues) {
        const cacheKey = `${conversationId || cleanDigits}:${issue.key}:${issue.status}`;
        const lastNotif = jiraNotifiedTimestamps.get(cacheKey) || 0;
        const now = Date.now();

        // Avoid repeated push notifications within 5 minutes for same issue/status
        if (now - lastNotif > 5 * 60 * 1000) {
          jiraNotifiedTimestamps.set(cacheKey, now);

          const board = issue.boardColumn || issue.status || 'A REVISAR';
          const notifId = `tars-jira-${issue.key}-${now}`;
          jiraNotificationLinks.set(notifId, {
            key: issue.key,
            url: issue.url || `https://belenergy.atlassian.net/browse/${issue.key}`,
            phone,
            customerName
          });

          // 1. Push Chrome Native Desktop Notification
          if (chrome.notifications && chrome.notifications.create) {
            chrome.notifications.create(notifId, {
              type: 'basic',
              iconUrl: 'icons/icon-128.png',
              title: `🎯 Kanban Jira Detectado: [${issue.key}]`,
              message: `Quadro/Status: "${board}"\nCliente: ${issue.customerName || customerName || phone}\n${issue.summary || ''}`,
              priority: 2,
              requireInteraction: true,
              buttons: [
                { title: 'Abrir no Jira' },
                { title: 'Ver no Solar Agenda' }
              ]
            }, (createdId) => {
              if (chrome.runtime.lastError) {
                console.warn('[TARS Jira Push Notification] Error:', chrome.runtime.lastError.message);
              } else {
                console.info('[TARS Jira Push Notification] Sent successfully:', createdId);
              }
            });
          }

          // 2. Broadcast to Hyperflow tab for in-page HUD badge
          if (senderTabId) {
            try {
              chrome.tabs.sendMessage(senderTabId, {
                type: 'TARS_JIRA_KANBAN_POPUP',
                issue,
                phone,
                customerName
              }, () => { if (chrome.runtime.lastError) {} });
          ...
          // 3. Broadcast to Solar Agenda tabs
          const appTabs = await findAllAppTabs();
          for (const tab of appTabs) {
            if (tab && tab.id) {
              try {
                chrome.tabs.sendMessage(tab.id, {
                  type: 'TARS_JIRA_KANBAN_DETECTED',
                  issue,
                  phone,
                  customerName
                }, () => { if (chrome.runtime.lastError) {} });
    ...
    return { ok: true, activeIssues, totalCount: data.totalCount };
  } catch (err) {
    console.warn('[TARS Jira Kanban] Error checking Jira by phone:', err);
    return { ok: false, error: String(err?.message || err) };
  }
}
...
if (chrome.notifications && chrome.notifications.onButtonClicked) {
  chrome.notifications.onButtonClicked.addListener((notifId, btnIdx) => {
    const driveMeta = driveNotificationLinks.get(notifId);
    if (driveMeta && driveMeta.url) {
      chrome.tabs.create({ url: driveMeta.url });
      return;
    }
    const meta = jiraNotificationLinks.get(notifId);
    if (!meta) return;
    if (btnIdx === 0 && meta.url) {
      chrome.tabs.create({ url: meta.url });
    } else if (btnIdx === 1) {
      findAllAppTabs().then(tabs => {
        if (tabs.length && tabs[0].id) {
          chrome.tabs.update(tabs[0].id, { active: true });
        } else {
          chrome.tabs.create({ url: dynamicAppOrigin || 'https://solar-agenda.vercel.app' });
        }
      });
    }
  });
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg && msg.type === 'TARS_UPLOAD_DRIVE_MEDIA') {
    uploadMediaToDrive(msg, sender?.tab?.id || null)
      .then(sendResponse)
      .catch(err => sendResponse({ ok: false, error: String(err?.message || err) }));
    return true;
  }

  if (msg && msg.type === 'TARS_CHECK_JIRA_KANBAN') {
    checkAndNotifyJiraKanban(msg, sender?.tab?.id || null)
      .then(sendResponse)
      .catch(err => sendResponse({ ok: false, error: String(err?.message || err) }));
    return true;
  }
  ...
  if (msg && msg.type === 'RELAY_READY') {
    if (sender?.tab?.id) {
      registeredAppTabs.add(sender.tab.id);
    ...
    if (msg.url) {
      try {
        ...
        dynamicAppOrigin = u.origin;
        // Dynamically update backend endpoint so telemetry and observer events go to the right server
        chrome.storage.local.set({ [TARS_OBSERVER_BACKEND_KEY]: `${u.origin}/api/tars/observer/events` });
      } catch (_) {}
    }
    sendResponse({ ok: true, registered: true });
    return true;
  }

  if (msg && msg.type === 'TARS_BRIDGE_PING') {
    if (sender?.tab?.id) {
      registeredAppTabs.add(sender.tab.id);
      appTabId = sender.tab.id;
    }
    const pingId = msg.pingId || 'ping-' + Date.now();
    console.info('[TARS BRIDGE] Received ping from Solar Agenda tab', { pingId, senderTab: sender?.tab?.id, url: msg.url });
    sendResponse({
      ok: true,
      pingId,
      receivedAt: Date.now(),
      version: '1.2.85',
      registeredTabsCount: registeredAppTabs.size,
      activeAppTabId: appTabId,
