...
(function(window){
  ...
  // ============================================================================
  // PREDICTIVE ANALYTICS & HISTORICAL SLA RESOLUTION MATRIX
  // ============================================================================
  // Evaluates risk level based on manufacturer, alarm code frequency, and historical resolution times
  const MFR_RISK_FACTORS = {
    'hoymiles': { baseRisk: 1.1, typicalResolutionHrs: 18, highRiskCodes: ['303', '304', '305', '306', '307', 'port 10000', 'grid volt'] },
    'deye':     { baseRisk: 1.3, typicalResolutionHrs: 28, highRiskCodes: ['f14', 'f18', 'f20', 'f22', 'f35', 'f41', 'grid err'] },
    'growatt':  { baseRisk: 1.2, typicalResolutionHrs: 24, highRiskCodes: ['102', '124', '129', 'error 102', 'pv isolation'] },
    'solis':    { baseRisk: 1.1, typicalResolutionHrs: 22, highRiskCodes: ['1053', 'un_g_v01', 'igbt_ov_temp'] },
    'fronius':  { baseRisk: 1.25, typicalResolutionHrs: 32, highRiskCodes: ['state 301', 'state 443', 'state 475'] },
    'foxess':   { baseRisk: 1.15, typicalResolutionHrs: 20, highRiskCodes: ['grid fault', 'iso fault', 'dci high'] }
  };

  function evaluatePredictiveSLARisk(item) {
    if (!item) return { isPredictiveCritical: false, score: 0, reason: '' };
    const mfrKey = String(item.equipment?.manufacturer || '').toLowerCase().trim();
    const problem = String(item.problem_summary || item.title || '').toLowerCase();
    const mfrConfig = MFR_RISK_FACTORS[mfrKey] || { baseRisk: 1.0, typicalResolutionHrs: 24, highRiskCodes: [] };

    let score = mfrConfig.baseRisk;
    const reasons = [];

    // Check high risk fault codes
    const hitCode = (mfrConfig.highRiskCodes || []).find(code => problem.includes(code.toLowerCase()));
    if (hitCode) {
      score += 0.8;
      reasons.push(`Código de alarme crítico detectado (${hitCode.toUpperCase()})`);
    }

    // Check repeat SN or frequency across cases
    const sn = String(item.equipment?.serial_number || '').trim();
    if (sn && Array.isArray(slaCases)) {
      const recurrences = slaCases.filter(c => c.id !== item.id && String(c.equipment?.serial_number || '').trim() === sn).length;
      if (recurrences > 0) {
        score += (recurrences * 0.4);
        reasons.push(`Reincidência de falha no SN (${recurrences} atendimento(s) anterior(es))`);
      }
    }

    // Check waiting manufacturer duration
    if (item.status === 'WAITING MANUFACTURER RESPONSE') {
      const waitStart = item.timeline?.filter(e => e.type === 'status_change' && e.detail?.includes('WAITING MANUFACTURER'))?.pop()?.timestamp;
      if (waitStart) {
        const hrsWaiting = (Date.now() - new Date(waitStart).getTime()) / 3600000;
        if (hrsWaiting > mfrConfig.typicalResolutionHrs) {
          score += 0.7;
          reasons.push(`Tempo de resposta do fabricante (${Math.round(hrsWaiting)}h) excedeu a média histórica (${mfrConfig.typicalResolutionHrs}h)`);
        }
      }
    }

    const isPredictiveCritical = score >= 2.0;
    return {
      isPredictiveCritical,
      score: Number(score.toFixed(2)),
      reason: reasons.join(' · ') || 'Padrão operacional estável'
    };
  }
  ...
  // Track notifications already sent to avoid duplicate desktop spam
  const sentSLANotifications = new Set();
  ...
  function computeSLAHealth(item) {
    ...
    const predictive = evaluatePredictiveSLARisk(item);
    ...
    // Critical imminent breach (< 2h remaining) or Predictive Critical Risk
    if (remainingMs <= 2 * 3600000 || predictive.isPredictiveCritical) {
      return {
        status: 'CRITICAL',
        label: predictive.isPredictiveCritical && remainingMs > 2 * 3600000
          ? `Risco Preditivo (${remainingHrs}h)`
          : `Iminente (${remainingHrs}h ${remainingMin}m)`,
        color: '#f87171',
        icon: '⚠️',
        remainingText: predictive.isPredictiveCritical && remainingMs > 2 * 3600000
          ? `Alerta Preditivo: ${predictive.reason}`
          : `Estouro iminente: restam ${remainingHrs}h ${remainingMin}m`,
        elapsedText,
        isImminent: true,
        isAtRisk: true,
        isPredictive: predictive.isPredictiveCritical,
        pctRemaining,
        pctElapsed,
        totalHrs,
        firstResponseHrs,
        riskLevel: predictive.isPredictiveCritical
          ? `Preditivo Alto: ${predictive.reason}`
          : 'Imediato (< 2h para estouro)',
  ...
  function updateSLAStats() {
    ...
    // Dispatch background push notifications for critical SLA cases
    if (window.Notify && typeof window.Notify.send === 'function' && activeCases.length > 0) {
      activeCases.forEach(c => {
        const h = computeSLAHealth(c);
        if (h.status === 'OVERDUE' || h.status === 'CRITICAL' || (h.predictive && h.predictive.isPredictiveCritical)) {
          const notifKey = `sla-${c.id}-${h.status}-${new Date().toDateString()}`;
          if (!sentSLANotifications.has(notifKey)) {
            sentSLANotifications.add(notifKey);
            const mfr = c.equipment?.manufacturer ? ` [${c.equipment.manufacturer}]` : '';
            const cust = c.customer?.name || 'Cliente';
            const title = h.status === 'OVERDUE'
              ? `🚨 SLA Violado: ${c.id}${mfr}`
              : (h.predictive && h.predictive.isPredictiveCritical)
                ? `⚡ Risco Preditivo Crítico: ${c.id}${mfr}`
                : `⚠️ Alerta SLA Iminente: ${c.id}${mfr}`;
            const body = `${cust} · ${h.remainingText}. Clique para abrir o caso.`;

            window.Notify.send(title, body, 'sla-breach-' + c.id, {
              caseId: c.id,
