...
(function (window) {
  ...
  async function validateObs(caseId, observationId) {
    try {
      const payload = { observationId, validated: true, validatedBy: 'Técnico Especialista' };
      let res = await fetch(`/api/tars/observer/cases/${encodeURIComponent(caseId)}/validate-observation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (!res.ok) {
        res = await fetch(`/api/tars/observer/cases?action=validate-observation&caseId=${encodeURIComponent(caseId)}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (!res.ok) {
        ...
        throw new Error(errJson.error || 'Falha ao validar observação');
      }
      const data = await res.json();
      currentCase = data.case;
      renderCaseModal();
  ...
  async function correctObsPrompt(caseId, observationId) {
    ...
    try {
      const payload = { observationId, correctedValue: newVal.trim(), reason: reason || 'Ajuste técnico', correctedBy: 'Técnico Especialista' };
      let res = await fetch(`/api/tars/observer/cases/${encodeURIComponent(caseId)}/correct-observation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (!res.ok) {
        res = await fetch(`/api/tars/observer/cases?action=correction&caseId=${encodeURIComponent(caseId)}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (!res.ok) {
        const errJson = await res.json().catch(() => ({}));
        throw new Error(errJson.error || 'Falha ao registrar correção');
      }
      const data = await res.json();
      currentCase = data.case;
      renderCaseModal();
  ...
  async function saveHumanAnalysis(caseId) {
    ...
    try {
      const payload = { visualNotes, technicianConclusion, updatedBy: 'Técnico Especialista' };
      let res = await fetch(`/api/tars/observer/cases/${encodeURIComponent(caseId)}/human-analysis`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (!res.ok) {
        res = await fetch(`/api/tars/observer/cases?action=human-analysis&caseId=${encodeURIComponent(caseId)}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (!res.ok) {
        const errJson = await res.json().catch(() => ({}));
        throw new Error(errJson.error || 'Falha ao salvar análise humana');
      }
      const data = await res.json();
      currentCase = data.case;
      renderCaseModal();
  ...
  async function submitCloseCase(caseId) {
    ...
    try {
      ...
      let res = await fetch(`/api/tars/observer/cases/${encodeURIComponent(caseId)}/close`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      ...
      if (!res.ok) {
        res = await fetch(`/api/tars/observer/cases?action=close&caseId=${encodeURIComponent(caseId)}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (!res.ok) {
        const errJson = await res.json().catch(() => ({}));
        throw new Error(errJson.error || 'Falha ao fechar caso');
      }
